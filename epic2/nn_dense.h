...
namespace utec::neural_network {
    ...
    public:
        ...
        Dense(size_t in_f, size_t out_f, WInit init_w_fun, BInit init_b_fun) {
            // Inicialización de los pesos con dimensiones (out_f, in_f)
            weights = Tensor<T, 2>(std::array<size_t, 2>{out_f, in_f});  // Tensor de pesos: out_f filas, in_f columnas
            ...
            biases = Tensor<T, 2>(std::array<size_t, 2>{out_f, 1});  // Tensor de sesgos: out_f filas, 1 columna
            init_b_fun(biases);}

        Tensor<T, 2> forward(const Tensor<T, 2>& x) override {
            input_cache = x;  // Guardamos las entradas para la retropropagación
            // X * W^G + b (usamos la transposición de X)
